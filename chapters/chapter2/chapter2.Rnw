...
\begin{figure}
	\caption{}
	\label{fig:TTR}
<<TTR, echo=FALSE, warning=FALSE, out.width=250, out.height=250>>=
...
text(ttr, labels=rownames(ttr), pos = 4, cex = .5)
@
\end{figure} 
...
traf_tfl <- read.tfl("../../tfls/galdos_trafalgar_spain.txt.tfl")
maria_tfl <- read.tfl("../../tfls/isaacs_maria_colombia.txt.tfl")
font_tfl <- read.tfl("../../tfls/Galdos_fontanaDeOro_spain.txt.tfl")
font_spc <- tfl2spc(font_tfl)
amalia_spc <- tfl2spc(amalia_tfl)
maria_spc <- tfl2spc(maria_tfl)
traf_spc <- tfl2spc(traf_tfl)
font.fzm <- lnre("fzm", font_spc, exact=FALSE)
amalia.fzm <- lnre("fzm", amalia_spc, exact=FALSE)
maria.fzm <- lnre("fzm", maria_spc, exact=FALSE)
traf.fzm <- lnre("fzm", traf_spc, exact=FALSE)
font.fzm.vgc <- lnre.vgc(font.fzm, (1:230000), variances=FALSE)
amalia.fzm.vgc <- lnre.vgc(amalia.fzm, (1:230000), variances=FALSE)
traf.fzm.vgc <- lnre.vgc(traf.fzm, (1:230000), variances=FALSE)
maria.fzm.vgc <- lnre.vgc(maria.fzm, (1:230000), variances=FALSE)
plot(maria.fzm.vgc, traf.fzm.vgc, amalia.fzm.vgc, font.fzm.vgc, legend=c('María', 'Trafalgar', 'Amalia', 'Fontana de oro'))
@
\end{figure}
...
source("../../code/compareVgc.R")
hapax <- compareSpcDir('../../tfls')
plot(hapax)
text(hapax, labels=rownames(hapax))
@
\end{figure}
